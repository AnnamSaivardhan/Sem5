...
class ModelClass(torch.nn.Module):
    ...
    def loss(self, pred, label):
        ...
        if self.args.model_type == "svm": 
            ##### TODO: Your code starts here#########
            one = torch.from_numpy(np.ones(label.shape))
            label = 2*label- one
            pred = torch.flatten(pred)
            label = label.to(torch.float32)
            x = one - pred*label
            loss = torch.sum(torch.nn.functional.relu(x))     
            ######Your code ends here##########
        elif  self.args.model_type == "nll":
            ##### TODO: Your code starts here###### 
            one = torch.from_numpy(np.ones(shape=pred.shape,dtype=np.float32)) 
            pred1 = one-pred
            pred = torch.log(pred)
            pred1 = torch.log(pred1)
            label=torch.reshape(label,(len(pred),1))
            label = label.to(torch.float32)
            label1 = one-label
            label = torch.flatten(label)
            pred = torch.flatten(pred)
            label1 = torch.flatten(label1)
            pred1 = torch.flatten(pred1)
            X = torch.dot(label,pred)+torch.dot(label1,pred1)
            loss = -1*(X)/pred.shape[0]  
            ######Your code ends here##########
        elif  self.args.model_type == "ranking":
            ##### TODO: Your code starts here###### 
            N = pred[label==0]
            ...
            N=N.repeat((len(P),1))
            N = torch.transpose(N,0,1)
            loss=torch.sum(torch.sign(torch.nn.functional.relu(N-P,inplace=True)))
            ######Your code ends here##########
        else: 
            raise NotImplementedError()
        return loss
    ...
    for data in loader:
        with torch.no_grad():
            ...
            if model.args.model_type == "svm": 
                ...
                ##### TODO: Your code starts here######  
                N = torch.cat(N,pred[label==0])
                P = torch.cat(P,pred[label==1])
                # P=P.repeat((len(N),1))
                # N=N.repeat((len(P),1))
                #P=torch.transpose(P,0,1)
                #eval_score +=torch.sum(torch.sign(torch.nn.functional.relu(P-N,inplace=True)))
                ######Your code ends here##########
            else: 
                raise NotImplementedError()
