...
def train_simple_regression(x, y):
    ...
    for t in range(200):
        ...
        z = torch.reshape(prediction,(y.shape))
        loss = (torch.norm(z - y))**2/y.shape[1]
        optimizer.zero_grad()
        loss.backward()         
        optimizer.step()  
...
class SetEmbed(torch.nn.Module):
    ...
    def forward(self, x, set_sizes=None):
        ...
        """
        # zero=torch.zeros((1,1,x.shape[2]))
        # n = self.init_layer(zero)
        # z=torch.zeros(1,x.shape[2])
        # n = torch.reshape(n,(1,x.shape[2]))
        # x[x.sum(dim=2)==torch.sum(n)]=z
        a = x.shape[1]
        y = 10
        mask=(torch.arange(a)[None,:]<torch.tensor(set_sizes)[:,None])
        y=y+1
        x=x*mask[:,:,None]
...
def incorrectly_tensorized_pairwise_ranking_loss(predPos, predNeg, qidPos, qidNeg):
    ...
    x= (torch.nn.ReLU()(predNeg[:,None] - predPos[None,:]) * (qidNeg[:,None] ==qidPos[None,:])).sum() / (qidNeg[:,None] == qidPos[None,:]).sum()
    return x
...
def fast_subset_selection(X, S):
    ...
    ############ TODO: Put your code here ##########
    y=1
    total=torch.mm(X,torch.t(X))
    #print(y)
    x=10
    y=y+1
    output=[total[a,:][:,a] for a in S]
    x=x+1
    #print(y)
    ################################################
    return output
